...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    model = KeypointRCNN(backbone, num_classes=num_classes,num_keypoints = 56)
    ...
    if load_pretrain_weights:
        ...
        weights_dict = torch.load("./keypointrcnn_resnet50_fpn_coco.pth", map_location="cpu")
        for k in list(weights_dict.keys()):
            # print(weights_dict.keys())
            if ("box_predictor" in k) or ("kps_score_lowres" in k):
                del weights_dict[k]
...
def main(args):
    ...
    kp_results_file = f"keypoint_results{now}.txt"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # write seg into txt
        with open(kp_results_file, "a") as f:
            # 写入的数据包括coco指标还有loss和learning rate
