...
class AnchorsGenerator(nn.Module):
    ...
    def __init__(self, sizes=(128, 256, 512), aspect_ratios=(0.5, 1.0, 2.0),strides = None):
    ...
    def forward(self, image_list, feature_maps):
        ...
        # time22 = time.time()
        # print("self.set_cell_anchors use time :########",time22-time11)
        # 根据提供的sizes和aspect_ratios生成anchors模板
        ...
        # time33 = time.time()
        # print("torch.cat used time :___",time33-time22)
        # Clear the cache in case that memory leaks.
        self._cache.clear()
        # time44 = time.time()
        # print("clear cache used time:********",time44-time33)
        return anchors
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        final_scores = []
        time2 = time.time()
        print("before nms used time: *(*(*(*(*(*(****",time2-time1)
        # 遍历每张图像的相关预测信息
        for boxes, scores, lvl, img_shape in zip(proposals, objectness_prob, levels, image_shapes):
            # 调整预测的boxes信息，将越界的坐标调整到图片边界上
            time1 = time.time()
            boxes = box_ops.clip_boxes_to_image(boxes, img_shape)
            print("clip_boxes_to_image used time:^^^^^^^^^^^",time.time()-time1)
            ...
            # time1 = time.time()
            # res = torch.ge(scores, self.score_thresh)
            # # res = scores>self.score_thresh
            time2 = time.time()
            # keep = torch.where(res)[0]
            # keep1 = torch.nonzero(res).squeeze()
            keep = torch.where(torch.ge(scores, self.score_thresh))[0]  # ge: >=
            time3 = time.time()
            # print("torch.ge used time:",time2-time1)
            print("torch.where used time:",time3-time2)
            # print("thresh scores used time : ^&^&^&^&^&",time2-time1)
            boxes, scores, lvl = boxes[keep], scores[keep], lvl[keep]
            
            # non-maximum suppression, independently done per level
            time1 = time.time()
            keep = box_ops.batched_nms(boxes, scores, lvl, self.nms_thresh)
            print("batch nms used time: _)_)_)_)_)_)_",time.time()-time1)
    ...
    def forward(self,
        ...
        time2 = time.time()
        print("rpn head use time:",time2-time1)

        # 生成一个batch图像的所有anchors信息,list(tensor)元素个数等于batch_size
        time1 = time.time()
        anchors = self.anchor_generator(images, features)
        time2 = time.time()
        print("anchor_generator use time:",time2-time1)
        # batch_size
        ...
        objectness, pred_bbox_deltas = concat_box_prediction_layers(objectness,
                                                                    pred_bbox_deltas)
        time2 = time.time()
        print("concat_box_prediction_layers use time:",time2-time1)
        ...
        time2 = time.time()
        print("decoder box used time: ####",time2-time1)
        ...
        time2 = time.time()
        print("nms use time:",time2-time1)
        losses = {}
        if self.training:
