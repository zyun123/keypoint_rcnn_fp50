...
class EvalCOCOMetric:
    ...
    def prepare_for_coco_keypoint(self, targets,outputs):
        for target, output in zip(targets, outputs):
            if len(output) == 0:
                continue
            ...
            self.img_ids.append(img_id)
            per_image_boxes = output["boxes"]
            per_image_keypoints = output["keypoints"].flatten(start_dim=1).tolist()
            per_image_classes = output["labels"].tolist()
            per_image_scores = output["scores"].tolist()

            res_list = []
            # 遍历每个目标的信息
            for kp, label, score in zip(per_image_keypoints, per_image_classes, per_image_scores):
                class_idx = int(label)
    ...
    def update(self, targets, outputs):
        if self.iou_type == "bbox":
            ...
            self.prepare_for_coco_keypoint(targets,outputs)
        else:
